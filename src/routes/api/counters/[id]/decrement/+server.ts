...
import { decrementCounter, parseCounterId } from '$lib/server/counters';
...
export const POST: RequestHandler = async ({ params }) => {
	...
	try {
		...
		console.error('[counters] decrement failed:', error);
		return json(
			{ error: { message: 'Failed to decrement counter', code: 'internal_error' } },
			{ status: 500 }
		);
	}
};
